...
static inline void updateVehicleBus(int busIdx, const CANParsers& cp, CarState& cs) {
    // Vehicle bus specific signals
    // BMS_energyStatus is multiplexed: mux 0 has pack/remaining energy, mux 1 has energyBuffer
    int muxIdx = static_cast<int>(cp.get(busIdx, "BMS_energyStatus", "BMS_energyStatusIndex"));
    if (muxIdx == 0) {
        cs.fullPackEnergy = cp.get(busIdx, "BMS_energyStatus", "BMS_nominalFullPackEnergy");
        cs.nominalEnergyRemaining = cp.get(busIdx, "BMS_energyStatus", "BMS_nominalEnergyRemaining");
    } else if (muxIdx == 1) {
        cs.energyBuffer = cp.get(busIdx, "BMS_energyStatus", "BMS_energyBuffer");
    }
    cs.maxRegenPower = cp.get(busIdx, "BMS_powerAvailable", "BMS_maxRegenPower");
    cs.maxDischargePower = cp.get(busIdx, "BMS_powerAvailable", "BMS_maxDischargePower");
    cs.packVoltage = cp.get(busIdx, "BMS_hvBusStatus", "BMS_packVoltage");
    cs.packCurrent = cp.get(busIdx, "BMS_hvBusStatus", "BMS_packCurrent");
    cs.packTMin = cp.get(busIdx, "BMS_bmbMinMax", "BMS_thermistorTMin");
    cs.packTMax = cp.get(busIdx, "BMS_bmbMinMax", "BMS_thermistorTMax");
    cs.odometer = cp.get(busIdx, "DI_odometerStatus", "DI_odometer");
}
...
public:
    void update(const CANParsers &cp, CarState &cs) override {
        ...
        cs.gear = cp.get(1, "DI_systemStatus", "DI_gear");
        cs.egoSpeed = cp.get(1, "DI_speed", "DI_uiSpeed");
        ...
        cs.leftBlindSpot = cp.get(1, "DAS_status", "DAS_blindSpotRearLeft");
        cs.rightBlindSpot = cp.get(1, "DAS_status", "DAS_blindSpotRearRight");
        cs.fusedSpeedLimit = cp.get(1, "DAS_status", "DAS_fusedSpeedLimit");
        cs.trafficLightColor = cp.get(1, "DAS_road", "DAS_trafficLightColor");
        cs.stopLineDist = cp.get(1, "DAS_road", "DAS_stopLineDist");

        // NOTE: In case of DashKit, we use this turn signal, because UI_warning is only on Party bus
        cs.leftBlinker = cp.get(2, "VCLEFT_lightStatus", "VCLEFT_turnSignalStatus") + 1.0;
        cs.rightBlinker = cp.get(2, "VCRIGHT_lightStatus", "VCRIGHT_turnSignalStatus") + 1.0;

        cs.laneDepartureWarning = cp.get(1, "DAS_status", "DAS_laneDepartureWarning");
        cs.sideCollisionWarning = cp.get(1, "DAS_status", "DAS_sideCollisionWarning");

        // TODO: Fix these
        cs.anyDoorOpen = cp.get(1, "UI_warning", "anyDoorOpen");
        cs.buckleStatus = cp.get(1, "UI_warning", "buckleStatus");

        updateVehicleBus(2, cp, cs);
    }
};