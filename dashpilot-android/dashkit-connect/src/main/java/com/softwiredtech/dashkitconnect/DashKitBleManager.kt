...
// Dedicated type so BLE connection failures group under their own crash-reporter
// issue instead of a generic java.lang.Exception.
class BleConnectionException(message: String) : Exception(message)

@SuppressLint("MissingPermission")
class DashKitBleManager(
    private val context: Context,
    private val telemetry: DashKitTelemetry = DashKitTelemetry.None,
) {

    companion object {
        private const val TAG = "DashKitBleManager"
        const val DEVICE_NAME = "DashKit"
    ...
    private val scanTimeoutRunnable = Runnable {
        ...
        telemetry.log("BLE: scan timed out (attempt $attemptCount/$MAX_ATTEMPTS)")
        stopScan()
    ...
    private val connectTimeoutRunnable = Runnable {
        ...
        telemetry.log("BLE: connect attempt timed out (attempt $attemptCount/$MAX_ATTEMPTS)")
    ...
    private val scanCallback = object : ScanCallback() {
        override fun onScanResult(callbackType: Int, result: ScanResult) {
            ...
            telemetry.log("BLE: found DashKit, connecting")
            stopScan()
        ...
        override fun onScanFailed(errorCode: Int) {
            ...
            telemetry.recordException(BleConnectionException("BLE scan failed (error $errorCode)"))
    ...
    private val gattCallback = object : BluetoothGattCallback() {
        override fun onConnectionStateChange(g: BluetoothGatt, status: Int, newState: Int) {
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                telemetry.log("BLE: GATT connected (bondState=${g.device.bondState})")
                gatt = g
                ...
                when (g.device.bondState) {
                    ...
                    BluetoothDevice.BOND_BONDING -> {
                        ...
                        if (!g.device.createBond()) {
                            ...
                            telemetry.recordException(BleConnectionException("Could not start pairing (createBond failed)"))
                ...
                if (!reachedConnected) {
                    ...
                    telemetry.log("BLE: link dropped before setup completed (status=$status, attempt $attemptCount/$MAX_ATTEMPTS)")
                ...
                if (!userInitiatedDisconnect) {
                    if (android.os.SystemClock.elapsedRealtime() < pairingWindowUntilMs) {
                        ...
                        telemetry.log("BLE: dropped for pairing window; auto-reconnect paused")
                    ...
                    if (!appInForeground) {
                        ...
                        telemetry.log("BLE: dropped while backgrounded; reconnect deferred to foreground")
                        _connectionState.value = ConnectionStatus.Disconnected
                        forEachListener { it.onDisconnected() }
                        return
                    }
                    ...
                    if (quickDrop && quickDropCount >= MAX_QUICK_DROPS && !bondResetDone &&
                        ...
                        telemetry.log("BLE: removing stale bond after $quickDropCount quick drops; re-pairing")
                        bondResetDone = true
                        quickDropCount = 0
                        removeBond(g.device)
                    ...
                    telemetry.recordException(BleConnectionException("Link dropped unexpectedly after connect (status=$status); auto-reconnecting"))
                    _connectionState.value = ConnectionStatus.Connecting
                    forEachListener { it.onDisconnected() }
        ...
        override fun onPhyUpdate(g: BluetoothGatt, txPhy: Int, rxPhy: Int, status: Int) {
            ...
            telemetry.log("BLE: PHY updated tx=$txPhy rx=$rxPhy status=$status")
        }
        ...
        override fun onServicesDiscovered(g: BluetoothGatt, status: Int) {
            if (status != BluetoothGatt.GATT_SUCCESS) {
                ...
                telemetry.recordException(BleConnectionException("Service discovery failed (status=$status)"))
            ...
            telemetry.log("BLE: connected (${g.services.size} services, mtu=$mtu)")
            attemptCount = 0
    ...
    private fun removeBond(device: BluetoothDevice): Boolean =
        try {
            ...
            telemetry.log("BLE: removeBond failed: ${e.message}")
            false
        }
    ...
    private fun startScan() {
        val adapter = (context.getSystemService(Context.BLUETOOTH_SERVICE) as android.bluetooth.BluetoothManager)
            ...
            telemetry.recordException(BleConnectionException("Bluetooth not available"))
            _connectionState.value = ConnectionStatus.Error("Bluetooth not available")
            return
        }
        val scanner = adapter.bluetoothLeScanner ?: run {
            Log.e(TAG, "BLE scanner not available")
            telemetry.recordException(BleConnectionException("BLE scanner not available (Bluetooth off?)"))
        ...
        try {
            ...
            telemetry.log("BLE: scan started (attempt $attemptCount/$MAX_ATTEMPTS)")
        } catch (e: SecurityException) {
            Log.e(TAG, "BLE scan failed — missing permission: ${e.message}")
            telemetry.recordException(BleConnectionException("BLE scan failed — missing permission: ${e.message}"))
            _connectionState.value = ConnectionStatus.Error("Missing BLE permission")
        } catch (e: Exception) {
            Log.e(TAG, "BLE scan failed: ${e.message}")
            telemetry.recordException(BleConnectionException("BLE scan failed: ${e.message}"))
