...
test("fast fixture passes: exit 0, named report, markdown step summary", () => {
  ...
  try {
    const result = runCli(
      ...
      { env: { ...process.env, GITHUB_STEP_SUMMARY: summaryPath, PERF_DURATION: "10000" } }
    );
...
test("slow fixture fails budgets (exit 1) and names the violation", () => {
  ...
  const result = runCli(["fixtures/slow-app"], { env: { ...process.env, PERF_DURATION: "10000" } });
...
test("missing contract exits 2 with a clear message", () => {
  const result = runCli(["fixtures/no-contract-app"], { env: { ...process.env, PERF_DURATION: "5000" } });
